...
void AbstractControllerApp::sendFlowModMessage(bool aggregateValue, ofp_flow_mod_command mod_com, const oxm_basic_match &match, uint32_t outport, TCPSocket * socket, int idleTimeOut =1 , int hardTimeOut=0){
    EV << "sendFlowModMessage" << '\n';
    numFlowMod++;

    socket->send(createFlowMod(aggregateValue,mod_com,match,outport,idleTimeOut,hardTimeOut));
}
...
OFP_Flow_Mod * AbstractControllerApp::createFlowMod(bool aggregateValue,ofp_flow_mod_command mod_com,const oxm_basic_match  &match, uint32_t outport, int idleTimeOut =1 , int hardTimeOut=0){
    ...
    // Aggregate
    if(aggregateValue){
        flow_mod_msg->setAggregateFactor(AGGREGATE_FACTOR);
    }
    else{
        flow_mod_msg->setAggregateFactor(1);
    }
    return flow_mod_msg;
}
