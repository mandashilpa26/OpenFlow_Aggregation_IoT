...
void LLDPBalancedMinHop::handlePacketIn(OFP_Packet_In * packet_in_msg){
    ...
    if(route.empty()){
        ...
        // Aggregate
        bool aggregateValue = headerFields.swInfo->getAggregate();
        sendFlowModMessage(aggregateValue,OFPFC_ADD, match, seg.outport, controller->findSocketFor(packet_in_msg),idleTimeout,hardTimeout);
        ...
        while(!route.empty()){
            ...
            if(socket != NULL){
                sendFlowModMessage(aggregateValue,OFPFC_ADD, match, seg.outport, socket,idleTimeout,hardTimeout);
            }
        }
