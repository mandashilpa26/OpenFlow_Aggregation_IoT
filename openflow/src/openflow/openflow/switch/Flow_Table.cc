...
void Flow_Table::addEntry(Flow_Table_Entry entry) {
    ...
    //EV<<" Max size of a list is:" << entryList.max_size();
}
...
Flow_Table_Entry* Flow_Table::lookup(oxm_basic_match &match) {
    ...
    for(auto iter =entryList.begin();iter != entryList.end();++iter){
        ...
        // check if aggregate timer has expired.
        if ((*iter).getAggregateExpiresAt() < simTime() && (*iter).getAggregateTimeout()!=0){
            (*iter).setAggregateExpiresAt((*iter).getAggregateTimeout()+simTime());
            /*
             * as the timer has expired, call for aggregate
             */
        }
        ...
        if (flow_fields_match(match, (*iter).getMatch(), (*iter).getMatch().wildcards)){
            //adapt idle timer filed if necessary
