...
void OF_Switch::handleMessage(cMessage *msg){
    ...
    /*
     * If the message comes from scheduleAt() function, then the msg->isSelfMessage() returns true.
     */

    if (msg->isSelfMessage()){
        /*
         * the switch receives a message to be processed
         * 1. connect message
         * 2. service time message (send to processQueueMessage( data plane or control plane message)).
         *
         */
       ...
       if (dynamic_cast<Open_Flow_Message *>(data_msg) != NULL) { //msg from controller
            ...
            switch (type){
                case OFPT_FEATURES_REQUEST:
                    ...
                    break;
                case OFPT_FLOW_MOD:
                    EV<< " calling flow mod in ofswitch"<<'\n';
...
void OF_Switch::handleFeaturesRequestMessage(Open_Flow_Message *of_msg){
    ...
    // Aggregate
    featuresReply->setAggregate(aggregate);
