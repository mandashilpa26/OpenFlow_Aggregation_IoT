...
Flow_Table_Entry::Flow_Table_Entry(OFP_Flow_Mod *flowModMsg){
    ...
    // Aggregate
//    aggregateTimeout = flowModMsg->getAggregateTime();
//    if(aggregateTimeout!=0){
//        aggregateExpiresAt = aggregateTimeout+simTime();
//    }
...
SimTime Flow_Table_Entry::getAggregateExpiresAt() const {
    return aggregateExpiresAt;
}

void Flow_Table_Entry::setAggregateExpiresAt(SimTime aggregateExpiresAt) {
    this->aggregateExpiresAt = aggregateExpiresAt;
}

double Flow_Table_Entry::getAggregateTimeout() const {
    return aggregateTimeout;
}


void Flow_Table_Entry::setAggregateTimeout(double aggregateTimeout){
